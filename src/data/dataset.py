...
from src.data.utils import multilabel_train_test_split
...
class Dataset(object):
    def __init__(self,
        ...
        if self.dataset_type == 'nano':
            ...
            raise NotImplementedError(
                "Please set dataset_type as raw, micro, or nano.")
    ...
    def prepare_test_data_and_prediction(self):
        ...
        subject_ids = pd.DataFrame(
            data=subjpath, columns=['filepath'], index=test_idx)
    ...
    def split_training_into_validation(self):
        ...
        #subject_ids = pd.read_csv(subjpath, index_col=0)
        subject_ids = pd.DataFrame(
            data=subjpath, columns=['filepath'], index=labels.index)
        for row in subject_ids.itertuples():
            subject_ids.loc[row.Index] = os.path.join(row.filepath, row.Index)

        # split
        X_train, X_val, y_train, y_val = multilabel_train_test_split(
            subject_ids, labels, size=val_size, min_count=1, seed=0)

        # check distribution is maintained
        dist_diff = (y_train.sum() / y_train.shape[0] -
                     y_val.sum() / y_val.shape[0]).sum()
        #print(dist_diff)
    ...
    def batches(self, verbose=False):
        ...
        while 1:
            ...
            x, failed = self._get_video_batch(
                x_paths, reduce_frames=reduce_frames, verbose=verbose)
            ...
            if len(failed) != 0 and verbose == True:
                print(
                    "\t\t\t*** ERROR FETCHING BATCH {self.batch_num}/{self.num_batches} ***"
                )
    ...
    def test_batches(self, verbose=False):
        ...
        while 1:
            ...
            x_paths = pd.DataFrame(
                data=[
                    os.path.join(test_dir, "{filename}") for filename in x_ids
                ],
                columns=['filepath'],
                index=x_ids)
            #print(x_paths)
    ...
    def _fill_video(self, video):
        ...
        filler_frames = np.zeros(shape=(num_to_fill, self.width, self.height,
                                        1))  # assumes grey
    ...
    def _test_batch_generator(self):
        ...
        assert (self.y_train.shape[0] -
                self.train_data_seen.sum().values[0]) < self.batch_size
